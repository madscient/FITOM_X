...
`profile`を指定した場合、FITOM_Xは2つの方法でプラグインにプロファイルを
伝える:

1. **環境変数方式**: DLLロードの**直前**に`profile_env`で指定した環境変数へ
   `profile`のパスをセットする。DLLロード時点（静的シングルトン初期化）で
   自身の設定ファイルを自動読み込みする設計のプラグイン(FitomEmuIF等)向け。
2. **明示的API方式**: DLLロード**後**、プラグインが`HWPlugin_SetProfile()`
   (Optional機能、`IHWPlugin.h`参照)をエクスポートしていれば、
   `profile`のパスを直接渡して呼び出す。呼び出しタイミングに依存せず
   確実にプロファイルを反映できる、より明示的な方式。

**両方式は併用される**。`HWPlugin_SetProfile()`を実装しているプラグインで
はこちらが確実に効果を持ち、未実装の旧プラグインでは単に無視され
(FITOM_X側はシンボル有無を確認し、無ければ何もしない。クラッシュしない)、
環境変数方式のみが効く。新規にHWプラグインを実装する場合は
`HWPlugin_SetProfile()`の実装を推奨する。

これにより、FITOM_Xのプロファイルを切り替えると、対応する
プラグイン側の設定ファイルも連動して切り替わり、再現性が保たれる。
(環境変数を指定せず、`HWPlugin_SetProfile()`も未実装のプラグインの場合、
FitomEmuIFであればDLLと同じディレクトリまたはカレントディレクトリの
`fmhwif_profile.json`が使われる、という具合にプラグイン自身のデフォルト
探索ルールに従う。)
