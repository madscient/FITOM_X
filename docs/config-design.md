...
エミュレーターバックエンド使用時の RtAudio 設定。ハードウェアのみ使用する場合は無視される。
...
| フィールド | デフォルト | 説明 |
|---|---|---|
| `api` | `"auto"` | RtAudio API 種別。`wasapi` / `asio` / `ds` / `alsa` / `pulse` / `core` / `auto` |
...
### MIDI チャンネルマップ (`channel_map`)

```json
"channel_map": [
  { "midi_ch": 1,  "device_index": 0, "poly": 4 },
  { "midi_ch": 2,  "device_index": 1, "poly": 1 },
  { "midi_ch": 10, "device_index": 2, "poly": 1 }
]
```

省略時は FITOM コアのデフォルト割り当て（デバイス順に ch1 から割り当て）を使用する。
...
```json
{
  ...
  "midi_inputs": ["UM-ONE"],
  "channel_map": [
    { "midi_ch": 1,  "device_index": 0, "poly": 6 },
    { "midi_ch": 2,  "device_index": 1, "poly": 8 },
    { "midi_ch": 3,  "device_index": 2, "poly": 6 },
    { "midi_ch": 10, "device_index": 0, "poly": 1 }
  ]
}
```
...
3. RtAudio ストリームを開く (エミュレーター使用時のみ)
