...
- `VoiceProcessor::onVolumeChange()`が全チップ(FM系含む)で未使用(実質デッドコード)。発音中のCC7(Volume)/CC11(Expression)変更が、`effectiveTL()`を経由する音量計算(FM系の`updateVolExp`、ADPCM-B/PCMD8/ADPCM-A等)へ即座に反映されない可能性がある。2026年7月、pcmbank/samplezonebankへのSwPatch紐づけ機能の調査中に発見。既存の全チップ共通の課題であり範囲が広いため、未対応のまま
