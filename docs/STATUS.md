...
- ADPCM-B(OPNB/OPNBB, `kOPNB_DeltaT`)のStart/Endアドレスがなお不正で、
  OPNA ADPCM-Bと同一パッチのはずがOPNBではノイズになる不具合が未解決
  (2026-07-24)。`addrShift`を8→2へ一時的に差し戻して検証したが改善せず、
  YMEngine(`extern/ymfm/src/ymfm_opn.cpp`の`ym2610::ym2610()`が
  `m_adpcm_b(intf, 8)`と明示指定、`ymfm_adpcm.cpp`の
  `adpcm_b_channel::address_shift()`はコンストラクタ引数が非0ならそれを
  無条件に返す実装)のソースで、YM2610/2610Bが256byte境界(shift=8)固定で
  あることを直接確認できたため8へ戻した(2は誤り。OPNAは`m_adpcm_b(intf)`
  でaddrshift省略=0となり、control2のROM/RAM・8bit DRAMビットが両方0の
  場合のみ2になる動的判定で、本FITOM_Xの実装は常に0固定のため2で正しい)。
  8に戻した状態でもノイズが続く場合、`addrShift`自体ではなく
  adpcm_packer側の実際のバイト整列を疑う必要がある。
  `config_schema/pcmbank.schema.json`の`boundary`フィールドは
  「FITOM_X側では検証に使わない情報欄」であるため、pcmbank.jsonに
  `"boundary": 256`と書いてあっても実データ(`.bin`内の各エントリの
  実際のstart_offset/padded_size)が本当に256byte境界に整列されている
  保証にはならない。`CYmDelta::registerVoice()`はaddrShiftに基づく
  非整列を検知すると`FITOM_LOG_WARN`(「is not aligned to a 256-byte
  boundary」)を出す実装になっているため、次の調査では実機ログに
  この警告が出ていないか確認すること
