...
- 外部パッチエディタ起動機能(ダブルクリック→キオスクモード起動)は実機でのダブルクリック確認済み(2026年7月、プロファイルパス解決バグ修正、上記表参照)。ただし`PatchManager::resolveTriple()`がVoiceGroup照合のために`devices[]`から一致するデバイスを線形探索する都合上、プロファイルの`devices[]`に対応するHWプラグインDLLが実際にロードできていないと(`Config::buildDevice()`がHWプラグイン未登録時に早期returnし`DeviceEntry`自体を作らないため)解決に失敗する点は未検証のまま残っている
- 外部パッチエディタとの内部用MIDIパイプ(`fitom_midi_pipe`)連携は未検証。`FITOM_BUILD_BACKEND_MIDI_PIPE`が既定OFF(DLL未ビルド)な上、同梱サンプルプロファイル(`config/profiles/*.profile.json`)はどれも`midi_backend`/`midi_inputs`をfitom_midi_pipe向けに設定していない。加えて`FITOMBridge::reopenMidiPorts()`は1プロファイルにつきMIDIバックエンドDLLを1つしか保持できないため、実機MIDIキーボード(rtmidi)とfitom_midi_pipeは同一プロファイル内で併用できない設計上の制約がある
