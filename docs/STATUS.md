...
- ADPCM-B(OPNB/OPNBB, `kOPNB_DeltaT`)のStart/Endアドレス境界調査中(2026-07-24)。
  OPNA ADPCM-Bと同一のパッチのはずが、OPNBでは同じ音にならないとの報告あり。
  直前のセッションで256byte境界(`addrShift=8`)に修正したが、原因切り分けのため
  いったん旧値の4byte境界(`addrShift=2`)へ差し戻し中(`ADPCM_new.cpp`の
  `kOPNB_DeltaT`定義コメント参照)。8/2のどちらが実チップの実際の挙動と
  一致するか、実機での確認が必要
