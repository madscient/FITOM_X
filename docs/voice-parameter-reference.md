...
**適用(FITOM→実機)はOPLLと同じ`updateVoice`+`updateKey`の2段階方式**
(2026年7月訂正: 以前は「OPL系は`updateVoice`のみで完結する」と誤って
記載していた。実際にはOPL系もOPLLと同じくキーオン/キーオフのたびに
`updateKey`が動的にEGT/RRを書き換える)。
...
**OPLLとの違い**: OPLLの`updateKey`は`SR>0`のときだけパーカッシブに
切り替える(`SR==0`ならキーオン中もEGT=サステインのまま)のに対し、
OPL系の`updateKey`は`SR`の値を見ずキーオン状態だけで無条件に切り替える。
詳細はOPLL専用セクション(下記)を参照。
...
RRレジスタを、OPL系と同じく**`updateVoice`と`updateKey`という2つの
関数に分けて**書き込む(2026年7月に検証・訂正)。ただしOPL系の
`updateKey`が`SR`の値を見ずキーオン状態だけで無条件にEGT/RRを
切り替えるのに対し、OPLLの`updateKey`は**`SR>0`のときだけ**パーカッシブに
切り替える(`SR==0`ならキーオン中もサステインのまま)という違いがある。
