...
異なり、発音中のEGTビット書き換えが期待どおりに効かない疑いがあるため、
切り分け目的で分離した)。詳細はOPLL専用セクション(下記)を参照。
...
> **なぜOPL系と方式が違うのか**: OPL系(`COPL`/`COPL3`)は従来どおり
> `updateKey`でEGT/RRを動的に書き換えるが、OPLLは実機のEG挙動がOPL系と
> 異なり、発音中のEGTビット書き換えが期待どおりに効かない疑いがある。
> 2026年8月、この切り分けのためOPLL系のみ静的変換へ変更した
> (OPLL/OPLL2/OPLLP/OPLLX/VRC7は`COPLL`を共有するため全て同じ挙動)。
