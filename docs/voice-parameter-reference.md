...
| 音量(計算値) | `0x90\|(ch*32)\|att` (ch0-2) / `0xF0\|att` (ch3=ノイズ) | **減衰量**を書く。0=最大音量/15=無音(他のPSG系と極性が逆) |
...
| 音量(計算値) | 0xA8-0xAC (ch0-4、下位4bit) | 0=無音/15=最大 |
| Fnum(TonePeriod) | 0xA0+ch*2(下位8bit)/0xA1+ch*2(上位4bit、12bit周期値) | 周期テーブル(`master/(32*freq)`)を12bitへスケーリング |
