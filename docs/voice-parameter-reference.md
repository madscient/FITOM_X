...
## SSGS (YMZ705) / SSGS2 (YMZ732) SSG互換部 — `CSSGS`（`CSSG`派生）

YM2149相当のSSGブロックを2系統内蔵し、合計6chを持つ。**音色データは
SSG(`VOICE_PATCH_SSG`)と完全に共有する** — レジスタ互換であり、追加された
パンポットは音色データではなくMIDIのCC#10で制御されるため。上表の
フィールドはすべてそのまま適用でき、レジスタアドレスだけがユニットごとに
`0x20`ずれる。YMZ732(SSGS2)はYMZ705(SSGS)の上位互換でレジスタマップは
完全に同一のため、同じドライバ・同じ音色データがそのまま使える(混在構成でも
同一`VoicePatchType`として束ねられる)。
...
- パンポットは4bit（`0`=左端 / `8`=中央 / `15`=右端）。`ChState::panpot`(-64〜+63)を
  四捨五入で0-15へ写す。
- SSGブロックの動作クロックはどちらのチップでも2.048MHzで、周期テーブルの
  基準もこの値。マスタークロックからの分周比だけがチップで異なる
  (SSGS: 4.096MHz÷2 または 6.144MHz÷3 / SSGS2: 12.288MHz÷6固定)。
