...
拡張モードでは同一の8bitアドレスがレジスタ`0x0D`のbit4でBank A/Bに多重化されます。8bitアドレスだけをキーにするシャドウでは両バンクを区別できないため、`CEPSG`は内部シャドウでBank Bを`0x100`以降へ分離して保持し、チップへ送る際に8bitへ戻します。レジスタダンプもこの表現をそのまま表示するので、**`0x000-0x00F`がBank A、`0x100-0x10F`がBank B**として読めます。

この`0x100`はドライバ内部の表現で、HW I/Fプラグインへは常に8bitアドレス+バンク選択の形でしか送られません(プラグイン契約の`a_high`とは無関係)。
