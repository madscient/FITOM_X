...
- **EGT/RRは静的変換（OPL系の動的書き換えを適用しない）**：2026年8月〜。
  ...
  **OPL系(`COPL`/`COPL3`)との違い**：OPL系は4.3節のとおりキーオン/キーオフ
  のたびにEGT/RRを動的に書き換える技法を使い続けるが、OPLLは実機のEG挙動が
  OPL系と異なり、発音中のEGTビット書き換えが期待どおりに効かない疑いがある
  （動的制御を入れた状態で音が想定どおりに減衰しない）。切り分けのため
  OPLL系のみ静的変換へ戻した。**チップファミリー間の一貫性という原則の
  例外にあたるため、実機検証で決着したらどちらかに揃えること。**
