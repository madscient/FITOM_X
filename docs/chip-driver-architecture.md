...
- **EGT/RR動的書き換え技法**：`COPL`/`COPL3`/`COPLRhythm`は、キーオン中は
  ...
  **【2026年7月、誤った"修正"を一度加えてから差し戻した経緯】** 一時、
  YM3812アプリケーションマニュアルの「PERCUSSIVE SOUND/NON-PERCUSSIVE
  SOUND」節(EGT=0がnon-percussive、EGT=1がpercussiveと明記)を根拠に
  0/1を逆転させたが、これは参照した資料側の記載誤りだった。YM2413
  アプリケーションマニュアルの「EGT (ENVELOPE TYPE): Select
  sustain/decay」節(EGT=1がsustained、EGT=0がdecayと明記、かつ図の
  波形もそちらの記述と整合)で誤りが判明し、旧FITOM由来の実装
  (EGT=0=decay/EGT=1=sustained)へ差し戻した。**今後EGTの0/1方向を
...
- **EGT/RR動的書き換え技法**：OPL3と同じ技法（キーオン中はEGT=0(decay)に
  ...
  EGT=0はdecay(KON中もRELEASE RATEが効き続ける)。2026年7月、別のデータシート
  [YM3812]の記載を誤って根拠にEGTの0/1を逆転させる修正を一度行ったが、
  YM2413側の資料および波形図と整合しないことが判明し差し戻した。詳細は
  4.3節OPL系の同項目を参照）。
