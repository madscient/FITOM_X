...
| `0x7A`-`0x7E` | (将来予約) | - | - |
| `0x7F` | 無音バンク(`VOICE_PATCH_SILENCE`) | 使用しない | 使用しない |

---

## 無音バンク (CC#0=0x7F)

`PatchManager::resolveTriple()`が、`hwBank`/`hwProg`もデバイスリストも
一切参照せずに「解決成功・ただし発音しない」を返す唯一の経路。
チップ種別に依存しないため、チップ族ごとに無音用の`HwPatch`を
用意しなくてよい。

「存在しないバンク/プログラムを指定する」こととは明確に区別される。
そちらは**解決失敗**であり、`CInstCh::progChange()`が直前のパッチを
維持するため無音にならない。確実に音を止めたい箇所、および
`ToneLayer`/`DrumNote`のプレースホルダには、未定義の番号ではなく
この値を使う。実装上の要点は
`docs/midi-implementation-status.md`の「無音バンク」節を参照。

**SF2直行パスとの関係**: 外部のMIDIシーケンサー向けインストゥルメント
定義ファイルには、SF2のバンク名/パッチ名をCC#0/CC#32空間の中で
表現するためにCC#0=`0x7F`を使う流儀がある。これはあくまで定義ファイル
側の便宜的な割り当てで、FITOM_X側の実装は持たない。
**FITOM_Xがこの値を受信してもSF2直行パスへは切り替わらない**——
SF2直行パスとネイティブ経路の振り分けはMPU×chの窓テーブルへの所属
だけで決まる(`docs/sf2-fluidsynth-integration.md`の③⑤参照)。窓に属する
チャンネルではCC#0はfluidsynthへ転送されないため、この値が無音バンクと
解釈されることもない(バンク選択は実際にはCC#32→`sf2_banks`のみで
行われる)。窓に属さないチャンネルで受信した場合は上記の無音バンクと
して扱われ、そのチャンネルは無音になる。
...
ただし2026年8月、`chokeGroups`の実装が事実上機能していなかったこと
(発音参照がゲートタイム満了時に破棄されるため、チョークが走る頃には
停止対象の記録が消えていた)が判明し、実現方式を再設計した。あわせて
**同一ノートの再打鍵は同じ物理チャンネルを取り直す**方式に変わり、
旧`fixed_ch`が持っていた「1ノート=1ボイス」という性質だけは、
ハードウェア非依存な形で復活している。詳細は同じ節の
「発音参照の寿命と消音手段」を参照。
