...
自身が持つ設計に変更された**（以前はレイヤードパッチ(`Patch`)単位で
...
| `0x7A`-`0x7E` | (将来予約) | - | - |
| `0x7F` | SF2バンク(外部シーケンサー用インストゥルメント定義ファイルでの便宜的な割り当て) | SF2バンクのインデックス(`sf2_banks[].bank`) | SF2側のpreset番号 |

**`0x7F`について**: 外部のMIDIシーケンサー向けインストゥルメント定義
ファイルが、SF2のバンク名/パッチ名をCC#0/CC#32空間の中で表現するために
使う値。**FITOM_Xがこの値を受信してもSF2直行パスへは切り替わらない**——
SF2直行パスとネイティブ経路の振り分けはMPU×chの窓テーブルへの所属
だけで決まる(`docs/sf2-fluidsynth-integration.md`の③⑤参照)。窓に属する
チャンネルではCC#0はfluidsynthへ転送されず、バンク選択は実際にはCC#32
(→`sf2_banks`)のみで行われる。窓に属さないチャンネルで受信した場合は
通常のバンクセレクトMSBとして`bankSelM_`に保持されるだけで、対応する
`VOICE_PATCH_*`が無いため音色解決に失敗し、直前の音色が維持される。
...
設計のため、`hwProg`(HwBank内の格納スロット番号)をそのままチャンネル
番号に使うことはできない(1チャンネルにつきHwBankスロットを1つしか
使えなくなってしまう)。代わりに、**HwPatch自身が持つ
`ext.rhythm_ch`**(対象チャンネル番号、0=HH/1=CYM/2=TOM/3=SD/4=BD)を
`resolveTriple()`が参照し、`ResolvedTriple::forcedCh`に設定する
(2026年7月、`fixed_ch`廃止に伴い新設)。未設定(255)または範囲外は
解決失敗として扱う(無音、警告ログ)。

`hwProg`(どの音色候補を選ぶか)と`ext.rhythm_ch`(その音色がどの
物理チャンネル向けか)は完全に独立した軸のため、同じ楽器(例:SD)に
対して複数の音色バリエーションをHwBank内の別スロットに用意できる
(HwPatch作成者が各スロットの`rhythm_ch`を正しく設定する責任を負う)。
詳細は`docs/chip-driver-architecture.md`の「VoicePatchType対応表」参照。
