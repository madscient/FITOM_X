...
└─────────┼───────────────────┼────────────────────────────────────┘
  ...
  │  (実機/エミュレータ共通、│   │  fitom_midi_rtmidi.dll/.so/.dylib│
  │   FITOM本体は区別しない)│   │  (RtMidi、Win/Linux/macOS共通)   │
  │  (IHWPlugin)          │   │  (IMidiPlugin)                  │
  │  FitomIFTest を DLL化  │   └─────────────────────────────────┘
  │  (実機)                │
  │  FitomEmuIF            │
  │  (FmEngine統合エミュレータ、│
...
### IMidiPlugin.h
RtMidi (WinMM / ALSA / CoreMIDI) を共通インターフェースで抽象化。
MIDI メッセージは MIDI 1.0 バイト列で統一する。

---

## RtMidi バックエンド DLL の注意事項

1. **third_party/rtmidi が submodule 必須** → `git submodule update --init --recursive`
   を実行していないと `backends/midi_rtmidi` の configure が `FATAL_ERROR` になる。
2. **CMake 3.24 以上が必要** → RtMidi 本体が要求するバージョン
   (FITOM本体のルート`CMakeLists.txt`もこれに合わせて3.24以上を要求する)。
3. **静的リンク** → RtMidi自体は `fitom_midi_rtmidi.dll`/`.so`/`.dylib` に
   静的リンクされ、追加のランタイムインストールは不要(旧WMS実装のような
   別途Runtime要件は無い)。
4. **Windows/Linux/macOS を1ソースでカバー** → プラットフォームAPI選択は
   RtMidi側がコンパイル時に自動判定する(Windows→WinMM、Linux→ALSA、
   macOS→CoreMIDI)。旧実装はWindows MIDI Services/WinMM/ALSAをそれぞれ
   個別のDLLとして持っていたが、2026年7月にこの単一DLLへ統合した。
...
{
  "profile_name": "スタジオ環境",
  "midi_backend": { "dll": "fitom_midi_rtmidi.dll" },
  "hw_plugins": [
